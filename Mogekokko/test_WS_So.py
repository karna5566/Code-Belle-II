...
clusterZernikeMVA = ROOT.RooRealVar("clusterZernikeMVA", "@0*(-1)",ROOT.RooArgList(clusterZernikeMVA))
...
AmeSame = ROOT.RooFFTConvPdf(f"AmeSame_{args.Type}", "AmeSame", clusterTiming, Gawr, Amelia)
...
AmeSame_timing = ROOT.RooAddPdf("AmeSame_{args.Type}","",ROOT.RooArgList(AmeSame,Function_mva),ROOT.RooArgList(f)) 
...
AmeSame_timing_nan = ROOT.RooAddPdf("AmeSame_{args.Type}","",ROOT.RooArgList(AmeSame,Gura),ROOT.RooArgList(f))
...
parameters = [mean, sigma,Sigma, Sigmaa,mean_mva,sigma_mva,sigmaa_mva]
if args.Type == 'l_h' : 
	Com_model = ROOT.RooProdPdf(f"Com_model_{args.Type}","",ROOT.RooArgList(AmeSame_timing,B_guass_mva))
	parameters += [c1_t,c2_t,f]
elif args.Type == 'nan':
	Com_model = ROOT.RooProdPdf(f"Com_model_{args.Type}","",ROOT.RooArgList(AmeSame_timing_nan,B_guass_mva))
	parameters += [coe1,f]
elif args.Type == 'gamma':
	Com_model = ROOT.RooProdPdf(f"Com_model_{args.Type}","",ROOT.RooArgList(AmeSame,B_guass_mva)) 
