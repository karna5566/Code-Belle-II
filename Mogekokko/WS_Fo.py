...
parser.add_argument('-m','--Method',nargs='?',help='Binned or Unbinned')
...
Amelia_gamma = w_gamma.pdf("Amelia_gamma")
...
if args.Method == 'Binned':
	#mc_sample = ROOT.RooDataHist("mc_s", "mc_s",ROOT.RooArgList(clusterTiming,clusterZernikeMVA),h_mc)
...
if args.Method == 'Binned':
	...
	EN = AmeSame_com.chi2FitTo(mc_sample_l_h,ll)
...
'''
'''
mean_shift = ROOT.RooRealVar("mean_shift", "mean_shift",0)
...
Ina = ROOT.RooGaussian("Ina","Ina",clusterTiming,MAEN,sIgma) # Smearing function
...
model_nan_data = ROOT.RooAddPdf("model_nan_data","model_nan_data",ROOT.RooArgList(InaAmeSame_nan,Gura_nan),ROOT.RooArgList(f1_nan))
...
AmeSame_com_data = ROOT.RooAddPdf("AmeSame_com_data","AmeSame_com_data",ROOT.RooArgList(InaAmeSame_gamma,model_nan_data,model_l_h),ROOT.RooArgList(rg_data,rn_mc,rl_mc))

if args.Method == 'Binned': 
	HoloEN = InaAmeSame.chi2FitTo(data_sample,ll)
	
else : 
	HoloEN = AmeSame_com_data.fitTo(data_sample,ROOT.RooFit.Hesse(True),ROOT.RooFit.Save(True))
...
AmeSame_com_data.plotOn(t_frame_data,ROOT.RooFit.Components("InaAmeSame_gamma"),ROOT.RooFit.LineStyle(7),ROOT.RooFit.LineColor(ROOT.kBlue),ROOT.RooFit.Name('gamma'))
AmeSame_com_data.plotOn(t_frame_data,ROOT.RooFit.Components("InaAmeSame_nan"),ROOT.RooFit.LineStyle(7),ROOT.RooFit.LineColor(ROOT.kRed),ROOT.RooFit.Name('nan'))
